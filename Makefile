...
EXAMPLE_CONTENT_FOLDER=${BACKEND_FOLDER}/src/collective/contact_behaviors/setuphandlers/examplecontent
...
i18n: $(VENV_FOLDER) ## Update locales
	...
	@uv run python -m collective.contact_behaviors.locales
...
test-coverage: $(VENV_FOLDER) ## run tests with coverage
	@uv run pytest --cov=collective.contact_behaviors --cov-report term-missing
